...
def _cosine_interpolate(
    start: np.ndarray, end: np.ndarray, time_steps: list[float]
) -> np.ndarray:
    """
...
class OpenArmBimanual:
    ...
    def __init__(
        ...
        self.bimanual_config = BiOpenArmFollowerConfig(
            right_arm_config=right_config, left_arm_config=left_config
        )
    ...
    def get_both_positions(self):
        return {
            "left_postion": self.get_left_position,
    ...
    def move_arms_to_target(
        self,
        right_list: list[float] | None = None,
        left_list: list[float] | None = None,
        ...
        time_steps = np.linspace(
            0.0, 1.0, num=int(CONTROL_RATE_HZ * _speed_to_duration(speed))
        )
        if left_list:
            arms.append(self.arms.left_arm)
            paths.append(
                _cosine_interpolate(self.get_left_position(), left_list, time_steps)
            )

        if right_list:
            arms.append(self.arms.right_arm)
            paths.append(
                _cosine_interpolate(self.get_right_position(), right_list, time_steps)
            )
    ...
    def replay_example(
        self, repo_id: str, episode: int, repo_path: Path, fps: int = 30
    ):
        dataset_config = DatasetReplayConfig(
            repo_id=repo_id, episode=episode, root=repo_path, fps=fps
        )
