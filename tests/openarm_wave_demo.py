...
def wave_j6(robot, base_position, wave_angle=30.0, num_waves=20, speed=60):
    ...
    for i in range(num_waves):
        ...
        move_smooth(
            robot,
            "right",
            base_position if i == 0 else wave_right,
            wave_left,
            duration=half_wave_duration,
        )
        # Wave right
...
Examples:
  ...
  python wave_demo.py --angle 40 --speed 90     # Faster, wider waves
    ...
    parser.add_argument(
        "--waves", type=int, default=5, help="Number of complete waves (default: 5)"
    )
    parser.add_argument(
        "--angle",
        type=float,
        default=30.0,
        help="Wave angle in degrees (default: 30.0)",
    )
    parser.add_argument(
        "--speed",
        type=float,
        default=30.0,
        help="Wave speed in degrees/second (default: 30.0)",
    )
