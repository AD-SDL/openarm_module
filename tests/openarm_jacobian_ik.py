...
class OpenArmJacobianIK:
    ...
    def _forward_kinematics(
        self, chain, joint_angles: np.ndarray
    ...
    def _compute_jacobian(
        self, chain, joint_angles: np.ndarray, epsilon: float = 1e-5
    ) -> np.ndarray:
    ...
    def _clamp_joints(self, joint_angles: np.ndarray) -> np.ndarray:
        ...
        for i in range(7):
            clamped[i] = np.clip(
                clamped[i], self.joint_limits[i, 0], self.joint_limits[i, 1]
            )
        return clamped
    ...
    def solve(
        ...
        for iteration in range(max_iterations):
            ...
            if iteration % 10 == 0 and iteration > 0:
                print(
                    f"  Iteration {iteration}: error = {error_norm * 1000:.2f}mm, step = {adaptive_step:.3f}"
                )
    ...
    def solve_right_arm(
        ...
        **kwargs,
        ...
        solution, converged, error = self.solve(
            self._right_chain, target_position, initial_joints, **kwargs
        )
    ...
    def solve_left_arm(
        ...
        **kwargs,
        ...
        solution, converged, error = self.solve(
            self._left_chain, target_position, initial_joints, **kwargs
        )
    ...
    def solve_right_arm_physical(
        ...
        **kwargs,
        ...
        urdf_solution = self.ik_solver.solve_right_arm(
            target_position, initial_joints=urdf_initial, **kwargs
        )
